...
def load_config(opt):
    """
    Load configuration from opt and return logging_config and output_root
...
def load_pipeline(opt):
    """
    Load a pipeline from opt and return it.
    ...
    Returns
    -------
    syncmvd : StableSyncMVDPipeline or StableSyncMVDPipelineXL
            The loaded pipeline
    """
    if opt.t2i_model == "SD1.5":
        ...
        if opt.checkpoint_path:
            vae = AutoencoderKL.from_single_file(
                opt.checkpoint_path, torch_dtype=torch.float16
            )
...
def load_cond_t2i_pipeline(unet, vae, controlnet, device):
    ...
    cond_t2i_pipeline.scheduler = DDPMScheduler.from_config(
        cond_t2i_pipeline.scheduler.config
    )

    return cond_t2i_pipeline


def camera_rendering(
    output_dir,
    mesh_processor,
    height,
    width,
    ...
    list_pil = mesh_processor.save_conditioning_images(
        conditioning_images, output_dir, cond_type=cond_type
    )
    return list_pil
...
def run_cond_t2i_pipeline(
    ...
    if output_dir:
        image.save(
            join(
                output_dir,
                f"generated_ip_image_{datetime.now().strftime('%d%b%Y-%H%M%S')}.png",
            )
        )
...
def run_experiment(runtime_cond_t2i=False):
    ...
    mesh_processor, recalculated, mesh_data_dir = initialize_mesh_processor(
        opt, output_root, mesh_path, syncmvd.camera_poses, syncmvd._execution_device
    )
    ...
    if runtime_cond_t2i:
        cond_t2i_pipeline = load_cond_t2i_pipeline(
            syncmvd.unet, syncmvd.vae, syncmvd.controlnet, syncmvd._execution_device
        )
        if recalculated:
            cond_image = camera_rendering(
                mesh_data_dir,
                mesh_processor,
                opt.latent_view_size * 8,
                opt.latent_view_size * 8,
                opt.max_batch_size,
                opt.guess_mode,
                render_cam_index=4,
                device=syncmvd._execution_device,
            )
        else:
            cond_image = load_cond_image(mesh_data_dir)
        ip_adapter_image = run_cond_t2i_pipeline(
            opt,
            cond_t2i_pipeline,
            cond_image,
            num_inference_steps=30,
