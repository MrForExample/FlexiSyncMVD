...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
	...
	def __call__(
		...
		positive_prompt_embed_dict = dict(zip(direction_names, [emb for emb in positive_prompt_embeds]))
		...
		positive_prompt_embeds = [azim_prompt(positive_prompt_embed_dict, pose) for pose in self.camera_poses]
		...
		if ip_adapter_image is not None:
			self.load_ip_adapter("h94/IP-Adapter", subfolder="models", weight_name="ip-adapter-plus_sd15.safetensors")
		...
		'''
		prompt_embeds_groups = {"positive": (positive_prompt_embeds, positive_image_embeds)}
		if do_classifier_free_guidance:
			prompt_embeds_groups["negative"] = (negative_prompt_embeds, negative_image_embeds)
		...
		with self.progress_bar(total=num_inference_steps) as progress_bar:
			for i, t in enumerate(timesteps):
				...
				for prompt_tag, two_prompt_embeds in prompt_embeds_groups.items():
					...
					for model_input_batch, prompt_embeds_batch, image_embeds_batch, down_block_res_samples_batch, mid_block_res_sample_batch, meta \
						in zip(model_input_batches, prompt_embeds_batches, image_embeds_batches, down_block_res_samples_list, mid_block_res_sample_list, self.group_metas):
		...
		mesh_processor.uvp.save_mesh(f"{self.result_dir}/textured.obj", result_tex_rgb.permute(1,2,0))
